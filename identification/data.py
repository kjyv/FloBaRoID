...
class Data:
    ...
    def removeLastSampleBlock(self):
        ...
        for k in self.measurements.keys():
            self.samples[k] = np.delete(
                ...
                list(
                    range(
                        self.num_selected_samples - self.opt["blockSize"],
    ...
    def selectBlocks(self):
        ...
        perc_cond = np.percentile(
            [cond for (b, bs, cond, linkConds) in self.seenBlocks],
            self.opt["selectBestPerenctage"],
        )
        ...
        while i < c:
            ...
            if (
                i < c - 1
                and np.abs(variances[sort_idx][i - 1] - variances[sort_idx][i + 1])
                < np.abs(variances[sort_idx][i + 1]) * dist
            ):
            ...
            if IMUlinVel is not None:
                ...
                if self.opt["waitForZeroAcc"]:
                    ...
                    if la.norm(IMUlinAccWorld[:, 0]) > 0.1:
                        print(
                            "Warning: proper base acceleration not zero at time 0 "
