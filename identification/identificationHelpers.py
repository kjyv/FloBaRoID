    ...
    def paramsFromiDyn2URDF(self, params):
        ...
        for i in range(0, self.n_params):
            if (i % 10 == 0):   #for each joint
                link_mass = params.getVal(i)

                #com
                com_x = params.getVal(i+1)
                com_y = params.getVal(i+2)
                com_z = params.getVal(i+3)
                params.setVal(i+1, com_x / link_mass)  #x of first moment -> x of com
                params.setVal(i+2, com_y / link_mass)  #y of first moment -> y of com
                params.setVal(i+3, com_z / link_mass)  #z of first moment -> z of com
                p_com = iDynTree.PositionRaw(params.getVal(i+1), params.getVal(i+2), params.getVal(i+3))

                #inertias
                rot_inertia_origin = iDynTree.RotationalInertiaRaw()
                #xx of inertia matrix w.r.t. link origin
                value = params.getVal(i+4)
                rot_inertia_origin.setVal(0, 0, value)
                #xy
                value = params.getVal(i+5)
                rot_inertia_origin.setVal(0, 1, value)
                rot_inertia_origin.setVal(1, 0, value)
                #xz
                value = params.getVal(i+6)
                rot_inertia_origin.setVal(0, 2, value)
                rot_inertia_origin.setVal(2, 0, value)
                #yy
                value = params.getVal(i+7)
                rot_inertia_origin.setVal(1, 1, value)
                #yz
                value = params.getVal(i+8)
                rot_inertia_origin.setVal(1, 2, value)
                #zz
                value = params.getVal(i+9)
                ...
                params.setVal(i+4, rot_inertia_com.getVal(0, 0))    #xx w.r.t. com
                params.setVal(i+5, rot_inertia_com.getVal(0, 1))    #xy w.r.t. com
                params.setVal(i+6, rot_inertia_com.getVal(0, 2))    #xz w.r.t. com
                params.setVal(i+7, rot_inertia_com.getVal(1, 1))    #xx w.r.t. com
                params.setVal(i+8, rot_inertia_com.getVal(1, 2))    #xy w.r.t. com
                params.setVal(i+9, rot_inertia_com.getVal(2, 2))    #xz w.r.t. com
