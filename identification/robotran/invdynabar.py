...
def invdynabar(Qq_out, q, qd, qdd, delta):
    ...
    CF19 = -ALS29*b93+ALS39*b92+K911*OMP19+K912*OMP29+K913*OMP39+OM29*(K913*OM19+\
        K923*OM29-Kd9*OM39)-OM39*(K912*OM19+K923*OM39+Kd9*OM29)
    CF29 = ALS19*b93+K912*OMP19+K923*OMP39+Kd9*OMP29-OM19*(K913*OM19+K923*OM29-Kd9*\
        OM39)+OM39*(K911*OM19+K912*OM29+K913*OM39)
    CF39 = -ALS19*b92+K913*OMP19+K923*OMP29-Kd9*OMP39+OM19*(K912*OM19+K923*OM39+Kd9*\
        OM29)-OM29*(K911*OM19+K912*OM29+K913*OM39)
    GS18 = GS19+BETA38*b83+BS18*br81
    GS28 = BETA48*br81+BETA68*b83+GS29*C9-GS39*S9
    GS38 = BETA78*br81+BS98*b83+GS29*S9+GS39*C9
    CF18 = CF19-ALS27*b83+K812*OMP28-Kd8*OMP18+Kr813*OMP38+OM28*(K823*OM28+Kd8*OM38+Kr813*OM18)- \
        OM38*(K812*OM18+K823*OM38+Kr822*OM28)-d[3,9]*(GS29*C9-GS39*S9)
    CF28 = ALS18*b83-ALS38*br81+CF29*C9-CF39*S9+GS19*d[3,9]+K812*OMP18+K823*OMP38+Kr822*OMP28- \
        OM18*(K823*OM28+Kd8*OM38+Kr813*OM18)+OM38*(K812*OM28-Kd8*OM18+Kr813*OM38)
    CF38 = ALS27*br81+CF29*S9+CF39*C9+K823*OMP28+Kd8*OMP38+Kr813*OMP18+OM18* \
        (K812*OM18+K823*OM38+Kr822*OM28)-OM28*(K812*OM28-Kd8*OM18+Kr813*OM38)
    GS17 = BETA27*br72+BS17*b71+GS18*C8+GS38*S8
    GS27 = GS28+BETA47*b71+BS57*br72
    GS37 = BETA77*b71+BETA87*br72-GS18*S8+GS38*C8
    CF17 = ALS37*br72+CF18*C8+CF38*S8+K712*OMP27+K713*OMP37+Kd7*OMP17+OM27*(K713*OM17+K723*OM27 \
        +Kr733*OM37)-OM37*(K712*OM17+K723*OM37-Kd7*OM27)
    CF27 = CF28-ALS37*b71+K712*OMP17+K723*OMP37-Kd7*OMP27-OM17*(K713*OM17+K723*OM27+\
        Kr733*OM37)+OM37*(K712*OM27+K713*OM37+Kd7*OM17)
    CF37 = -ALS17*br72+ALS27*b71-CF18*S8+CF38*C8+K713*OMP17+K723*OMP27+Kr733*OMP37+OM17* \
        (K712*OM17+K723*OM37-Kd7*OM27)-OM27*(K712*OM27+K713*OM37+Kd7*OM17)
    GS16 = BETA36*br63+BS16*b61+GS17*C7-GS27*S7
    GS26 = BETA46*b61+BETA66*br63+GS17*S7+GS27*C7
    GS36 = GS37+BETA76*b61+BS96*br63
    CF16 = -ALS26*br63+CF17*C7-CF27*S7+K612*OMP26-Kd6*OMP16+Kr613*OMP36+OM26*(K623*OM26+Kd6*\
        OM36+Kr613*OM16)-OM36*(K612*OM16+K623*OM36+Kr622*OM26)-d[3,7]*(GS17*S7+GS27*C7)
    CF26 = ALS16*br63-ALS36*b61+CF17*S7+CF27*C7-GS37*d[1,7]+K612*OMP16+K623*OMP36+Kr622*OMP26- \
        OM16*(K623*OM26+Kd6*OM36+Kr613*OM16)+OM36*(K612*OM26-Kd6*OM16+Kr613*OM36)+d[3,7]*(GS17*C7-GS27*S7)
    CF36 = CF37+ALS26*b61+K623*OMP26+Kd6*OMP36+Kr613*OMP16+OM16*(K612*OM16+K623*OM36+Kr622*OM26)- \
        OM26*(K612*OM26-Kd6*OM16+Kr613*OM36)+d[1,7]*(GS17*S7+GS27*C7)
    GS15 = BETA25*br52+BS15*b51+GS16*C6+GS36*S6
    GS25 = GS26+BETA45*b51+BS55*br52
    GS35 = BETA75*b51+BETA85*br52-GS16*S6+GS36*C6
    CF15 = ALS35*br52+CF16*C6+CF36*S6-GS26*d[3,6]+K513*OMP35+Kd5*OMP15+Kr512*OMP25+OM25* \
        (K513*OM15+Kr523*OM25+Kr533*OM35)-OM35*(-Kd5*OM25+Kr512*OM15+Kr523*OM35)
    CF25 = CF26-ALS35*b51-Kd5*OMP25+Kr512*OMP15+Kr523*OMP35-OM15*(K513*OM15+Kr523*OM25+Kr533*\
        OM35)+OM35*(K513*OM35+Kd5*OM15+Kr512*OM25)-d[1,6]*(-GS16*S6+GS36*C6)+d[3,6]*(GS16*C6+GS36*S6)
    CF35 = -ALS15*br52+ALS25*b51-CF16*S6+CF36*C6+GS26*d[1,6]+K513*OMP15+Kr523*OMP25+Kr533*\
        OMP35+OM15*(-Kd5*OM25+Kr512*OM15+Kr523*OM35)-OM25*(K513*OM35+Kd5*OM15+Kr512*OM25)
    GS14 = BETA24*b42+BETA34*br43+GS15*C5-GS25*S5
    GS24 = BETA64*br43+BS54*b42+GS15*S5+GS25*C5
    GS34 = GS35+BETA84*b42+BS94*br43
    CF14 = qdd[4]*Kr411-ALS24*br43+ALS34*b42+CF15*C5-CF25*S5+K412*OMP24+K413*OMP34+OM24*\
        (qd[4]*K413+K423*OM24-Kd4*OM34)-OM34*(qd[4]*K412+K423*OM34+Kd4*OM24)-d[3,5]*(GS15*S5+GS25*C5)
    CF24 = -qd[4]*(qd[4]*K413+K423*OM24-Kd4*OM34)+qdd[4]*K412+ALS14*br43+CF15*S5+CF25*C5+\
        K423*OMP34+Kd4*OMP24+OM34*(qd[4]*Kr411+K412*OM24+K413*OM34)+d[3,5]*(GS15*C5-GS25*S5)
    CF34 = CF35+qd[4]*(qd[4]*K412+K423*OM34+Kd4*OM24)+qdd[4]*K413-ALS14*b42+K423*OMP24-Kd4*\
        OMP34-OM24*(qd[4]*Kr411+K412*OM24+K413*OM34)
