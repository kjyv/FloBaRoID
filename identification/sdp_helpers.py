...
def to_cvxopt(objective_func, lmis, variables, objective_type="minimize", split_blocks=True):
    ...
    obj_coeffs = lmi_sdp.objective_to_coeffs(objective_func, variables, objective_type)
    ...
    for LMis, LM0 in lmi_coeffs:
        Gs.append(matrix([(-LMi).flatten().astype(float).tolist() for LMi in LMis]))
...
def cvxopt_dsdp5(
    ...
    c, Gs, hs = to_cvxopt(objf, lmis, variables)
    cvxopt.solvers.options["dsdp"] = {
        "DSDP_GapTolerance": epsilon_sdptol,
        "DSDP_Monitor": 10,
    }
    if wide_bounds:
